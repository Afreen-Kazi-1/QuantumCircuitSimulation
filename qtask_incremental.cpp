...
std::vector<int> partitions_touched_by_gate(const SimContext &ctx, const Gate &g)
{
    ...
    for (int pid = 0; pid < (int)ctx.partitions.size(); ++pid)
    {
        ...
        bool touch = false;
        for (size_t offset = 0; offset < p.length && !touch; ++offset)
        {
            size_t idx = p.start_idx + offset;
            if (g.type == H_GATE)
            {
                // H on qubit q mixes idx with idx ^ (1<<q), so partition is touched if it contains either idx or the partner.
                touch = true; // H acts on every amplitude; conservatively mark all partitions that have any amplitude (that's all partitions with any nonzero range)
            }
            else if (g.type == CNOT_GATE)
            {
                int control = g.qubits[0];
                int target = g.qubits[1];
                // CNOT swaps amplitude positions when control bit is 1. For correctness, mark if partition contains idx such that either idx or partner is in domain (always true if any idx exists).
                // To be more precise: mark partition if exists idx in partition where ((idx >> control) &1) in {0,1} (always), so essentially every partition can be involved.
                // Instead we check whether there exists idx in partition where either idx or partner is within domain - we simply mark partition if partition has any amplitude (safe).
                touch = true;
            }
        }
        if (touch)
    ...
    // For H these are all partitions; for small demo that's ok.
    return out;
}
